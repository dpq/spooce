...
*
**** Warning: contains local adjustments! ****
--------------------------------------------------------------------*/
...
function px2em(i, settings) {
    ...
    if (settings.scope == 'body' && $.browser.msie && (parseFloat($('body').css('font-size')) / getWindowWidth()).fixed(1) > 0.0) {
        var calcFontSize = function() {
            return (parseFloat($('body').css('font-size'))/getWindowWidth()).fixed(3) * 16;
        };
    ...
    if (settings.output == 'number') {
        var result = (settings.reverse === true) ? (pxVal*scopeVal).fixed(2) : (pxVal/scopeVal).fixed(2);
        return result;
    }
    else if (settings.output == 'string') {
        var result = (settings.reverse === true) ? (pxVal * scopeVal).fixed(2) + 'px' : (pxVal / scopeVal).fixed(2) + 'em';
        return result;
    }
}
...
if (!opt.package["portal"]) {
    opt.package["portal"] = {};
}

opt.package["portal"]["1"] = function() {
    var appid;
    ...
    this.global2local = function(coord) {
        return {
            "x": ((coord.x/10).fixed(1) - (centerPosition.x/10).fixed(1) + px2em(1.5*portalWidth) - px2em($(this.htmlAnchor).css("left"))).fixed(1),
            "y": ((coord.y/10).fixed(1) - (centerPosition.y/10).fixed(1) + px2em(1.5*portalHeight) - px2em($(this.htmlAnchor).css("top"))).fixed(1)
        };
    };

    this.local2global = function(window) {
        return {
            "x": (parseFloat(window.css("left")) - px2em(1.5*portalWidth) + px2em($(this.htmlAnchor).css("left"))).fixed(1)*10 + centerPosition.x,
            "y": (parseFloat(window.css("top")) - px2em(1.5*portalHeight) + px2em($(this.htmlAnchor).css("top"))).fixed(1)*10 + centerPosition.y
        };
    };
    ...
    /* These variables are necessary in order to keep down the number of times we modify the location.hash */
    var panHash, panTimeout, panClear = true;
    var zoomHash, zoomTimeout, zoomClear = true;
    
    
    /* This object contains references to all windows currently open in the portal, */
    var windows = {};
        ...
        if (message.windows) {
            for (var w in message.windows) {
                ...
                if (message.windows[w].action == "open") {
                    ...
                    if ((message.windows[w].meta && message.windows[w].meta.appcode && message.windows[w].meta.appcode == "portal")) {
                        $(window).css({"border": "1px dotted black"});
                    }
                    else {
                        $(window).css({"background-color": "gray"});
                        //TODO run the application specified in meta instead
                    }
    ...
    this.resetBoardSize = function(isInit) {
        if (!isInit) {
            $(this.htmlAnchor).css("left", parseInt($(this.htmlAnchor).css("left")) - (portalWidth - mkPortalWidth.call(this))*1.5 + "px");
            $(this.htmlAnchor).css("top", parseInt($(this.htmlAnchor).css("top"))  - (portalHeight - mkPortalHeight.call(this))*1.5 + "px");
        }
        ...
        //TODO Add this for IE
        //$(this.board).css({"left": -portalWidth + "px", "top": -portalHeight + "px"});
        if (isInit) {
    ...
    function portalWidth_div() {
        return parseInt($(this.node).css("width"));
    }
